...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (groupId && groupId !== "all") {
      whereCondition = and(
        eq(finalTasks.status, "graded"),
        eq(finalTasks.groupId, groupId)
      );
    } else {
      whereCondition = eq(finalTasks.status, "graded");
    }

    // Fetch graded tasks with all related data
    ...
    console.error("[API_ADMIN_FINAL_GRADING_GRADED_TASKS_GET] Error:", error);
    return NextResponse.json(
      { error: "Failed to fetch graded tasks" },
      { status: 500 }
    );
  }
}
